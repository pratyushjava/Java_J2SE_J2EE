...
resultSet type : forward type(ResultSet.TYPE_FORWARD_ONLY) or scrollable(ResultSet.TYPE_SCROLL_INSENSITIVEOR ResultSet.TYPE_SCROLL_SENSITIVE) : 
...
3. How to set IN params of PST?(to be invoked  : in event listener : after user gives i/p)
...
1. Create CST
API : Connection i/f
public CallableStatement prepareCall(String invocationSyntax) throws SE
...
3. Register OUT / IN OUT params (i.e specify the JDBC data type of the OUT param to JVM)
...
Objective : Using scanner : accept sid,did,amt for funds transfer, exec the st.proc & disp
the results.  ---> implemented in TestStoredProc.java
...
This placess the RST cursor on the newly created row.
...
1. Database meta data : holds the info like : DB version,DB drvr version, Tx are supported or not, scrollable/updateble rsts, names of all tables from DB.....
To get D.M.D
API : Connection i/f
